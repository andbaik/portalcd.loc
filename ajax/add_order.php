...
try {
    ...
    $query = $pdo->prepare("INSERT regulations SET `id_railways`=?, `date_given`=?, `number_regulations`=?, `path_file`=?, `who_given`=?, `what_violation`=?, `object`=?, `term_date`=?, `id_user_new`=?,`ip_add_new`=?, `date_new`=?");
    $query->execute([$id_railways, $date_given, $num_reg . '/' . $date_shot, $file_path, $who_given, $what_violation, $object, $term_date, $id_user, $ip_add_new, $date]);
...
<b>Дата:</b> $date_given  № $num_reg  выдано $who_given<br>
<b>Нарушения:</b> $what_violation<br><br>
<b>Срок устранения:</b>$term_date<br>
<i>Посмотреть можно по адресу: <a>$sait$file_path</a></i>
";
